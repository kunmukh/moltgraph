...
### Option A: delete all nodes/edges (keeps schema)
...
### Option B: wipe Docker volumes (factory reset)

```bash
docker compose down
docker volume rm moltbook_neo4j_data moltbook_neo4j_logs
docker compose up -d neo4j
docker compose run --rm crawler python -m scripts.init_db
```

> Volume names may differ. Use `docker volume ls | grep neo4j` to confirm.
